...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            # Log request error
            logger.error(
                f"Request failed: {request.method} {request.url.path} - {e.__class__.__name__}: {str(e)}"
            )
...
async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
    ...
    # Startup
    logger.info(
        f"Starting ManipulatorAI application - Version: {settings.app_version}, Environment: {settings.environment}"
    )
    ...
    try:
        ...
        startup_duration = (time.time() - startup_start) * 1000
        logger.error(
            f"Application startup failed after {startup_duration:.2f}ms: {e.__class__.__name__}: {str(e)}"
        )
        raise
    ...
    try:
        ...
        shutdown_duration = (time.time() - shutdown_start) * 1000
        logger.error(
            f"Application shutdown encountered errors after {shutdown_duration:.2f}ms: {e.__class__.__name__}: {str(e)}"
        )
...
def create_application() -> FastAPI:
    ...
    async def validation_exception_handler(
        ...
        logger.warning(
            f"Request validation failed: {request.method} {request.url.path} - {len(exc.errors())} errors"
        )
    ...
    async def global_exception_handler(request: Request, exc: Exception) -> JSONResponse:
        ...
        logger.error(
            f"Unhandled exception: {exc.__class__.__name__}: {str(exc)} - {request.method} {request.url.path}"
        )
    ...
    async def detailed_health_check() -> JSONResponse:
        ...
        # Cast to dict to satisfy type checker since checks is known to be a dict
        checks = health_data["checks"]
        if not isinstance(checks, dict):
            raise ValueError("health_data['checks'] must be a dictionary")

        unhealthy_services = [service for service, status in checks.items() if status != "healthy"]
    ...
    logger.info(
        f"FastAPI application created successfully - {settings.app_name} v{settings.app_version} ({settings.environment})"
    )
...
def main() -> None:
    ...
    """
    logger.info(
        f"Starting ManipulatorAI server - {settings.host}:{settings.port} ({settings.environment})"
    )
